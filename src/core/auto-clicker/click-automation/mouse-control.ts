...
export class MouseControl {
  ...
  async move(x: number, y: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      exec(`powershell -Command "${psCommand}"`, (error: Error | null, stdout: string, _stderr: string) => {
        if (error) {
  ...
  async click(config: ClickConfig): Promise<ClickResult> {
  ...
  private async performSingleClick(button: string, _coordinates: { x: number; y: number }): Promise<void> {
    return new Promise((resolve, reject) => {
      const buttonFlags = {
      ...
      const flags = buttonFlags[button as keyof typeof buttonFlags];
      if (!flags) {
  ...
  private async performDoubleClick(button: string, coordinates: { x: number; y: number }): Promise<void> {
  ...
  private async performHoldClick(button: string, _coordinates: { x: number; y: number }, duration: number): Promise<void> {
    return new Promise((resolve, reject) => {
      const buttonFlags = {
        left: { down: 0x02, up: 0x04 },
        right: { down: 0x08, up: 0x10 },
  ...
  async testClick(): Promise<boolean> {
    try {
